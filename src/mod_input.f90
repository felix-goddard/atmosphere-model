...
contains
    ...
    function read_initial_file() result(initial_nc)
        ...
        if (.not. all(points(2:xsize) - points(1:xsize-1) == dx)) then
            write (log_str, '(a)') 'Spacing of x-coordinate points in initial condition netCDF is not uniform.'
            call logger % fatal('init_prognostic_fields', log_str)
            call abort_now()
        end if
        ...
        if (.not. all(points(2:ysize) - points(1:ysize-1) == dy)) then
            write (log_str, '(a)') 'Spacing of y-coordinate points in initial condition netCDF is not uniform.'
            call logger % fatal('init_prognostic_fields', log_str)
            call abort_now()
        end if
