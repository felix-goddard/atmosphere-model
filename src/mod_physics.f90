module mod_physics
   ...
   use mod_constants, only: gravity, kappa, hydrostatic_constant, &
                            reference_pressure, dry_heat_capacity
   ...
   subroutine calculate_physics_tendencies()
      ...
      do k = 1, config%nlev
         gz(is + 1:ie - 1, js + 1:je - 1, k + 1) = &
            ...
            + hydrostatic_constant*pt(is + 1:ie - 1, js + 1:je - 1, k)*( &
            pkap(is + 1:ie - 1, js + 1:je - 1, k) &
         ...
         do k = 1, config%nlev
            ...
            pt_adj(k) = (energy(k) - gz_adj(k) &
                        ...
                        /(dry_heat_capacity*playkap(i, j, k) &
                          /(reference_pressure**kappa) &
                          + .5*hydrostatic_constant &
                          *(pkap(i, j, k) - pkap(i, j, k + 1)))

            gz_adj(k + 1) = gz_adj(k) + hydrostatic_constant*pt_adj(k) &
                            *(pkap(i, j, k) - pkap(i, j, k + 1))
         end do
